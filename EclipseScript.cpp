...
public:
    ...
    {
        EclipseConfig::GetInstance().Initialize(reload);

        if (EclipseConfig::GetInstance().IsEclipseEnabled() && !reload)
        {
            ECLIPSE_LOG_INFO("[Eclipse]: Eclipse Lua Engine starting up...");

            EclipseScriptLoader::LoadScriptPaths();

            EclipseSolState* globalEngine = EclipseStateManager::GetInstance().GetGlobalState();
            bool isInitialized = globalEngine->IsInitialized();
            if (isInitialized)
            {
                ECLIPSE_LOG_INFO("[Eclipse]: Eclipse Global Lua Engine initialized");
                EclipseStateManager::GetInstance().RunAllScripts();
            }
            else
            {
                ECLIPSE_LOG_INFO("[Eclipse]: Eclipse Global Lua Engine failed to initialize");
            }
        }
    }
